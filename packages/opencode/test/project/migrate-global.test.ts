...
function seed(opts: { id: SessionID; dir: string; project: ProjectNs.ID }) {
  const now = Date.now()
...
describe("migrateFromGlobal", () => {
  it.live("migrates global sessions on first project creation", () =>
    Effect.gen(function* () {
      ...
      expect(pre.id).toBe(ProjectNs.ID.global)

      // 2. Seed a session under "global" with matching directory
      const id = legacySessionID()
      yield* seed({ id, dir: tmp, project: ProjectNs.ID.global })
      ...
      expect(real.id).not.toBe(ProjectNs.ID.global)
  ...
  it.live("migrates global sessions even when project row already exists", () =>
    Effect.gen(function* () {
      ...
      expect(project.id).not.toBe(ProjectNs.ID.global)
  ...
  it.live("does not claim sessions with empty directory", () =>
    Effect.gen(function* () {
      ...
      const id = legacySessionID()
      yield* seed({ id, dir: "", project: ProjectNs.ID.global })
      ...
      expect(row).toBeDefined()
      expect(row!.project_id).toBe(ProjectNs.ID.global)
    }),
  )
  ...
  it.live("does not steal sessions from unrelated directories", () =>
    Effect.gen(function* () {
      ...
      const id = legacySessionID()
      yield* seed({ id, dir: "/some/other/dir", project: ProjectNs.ID.global })
