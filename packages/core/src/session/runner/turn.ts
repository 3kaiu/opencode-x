  ...
  const runTurnAttempt = Effect.fn("SessionRunner.runTurn")(function* (
    ...
    return yield* Effect.uninterruptibleMask((restore) =>
      Effect.gen(function* () {
        ...
        // "unknown" finished step so the turn is not silently dropped. The
        // projector prunes the assistant message when such a step produced no
        // content, so live consumers still see the step while durable history
        // stays free of phantom empty messages.
