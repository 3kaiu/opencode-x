...
describe("SessionRunnerLLM", () => {
  ...
  it.effect("streams one request with registry definitions from chronological V2 user history", () =>
    Effect.gen(function* () {
      ...
      expect(requests[0]?.tools.map((tool) => tool.name)).toEqual(["echo", "defect", "get_tool_schema"])
  ...
  it.effect("automatically compacts into a completed summary and retained recent turn", () =>
    Effect.gen(function* () {
      ...
      expect(userTexts(requests[1])[0]).toContain(`[User]: ${"Recent exact request ".repeat(188)}`)
  ...
  it.effect("does not derive a title for sessions with an explicit or parent title", () =>
    Effect.gen(function* () {
      ...
      yield* db
        .update(SessionTable)
        .set({ title: "Explicit title", parent_id: Session.ID.make("ses_parent") })
        .where(eq(SessionTable.id, sessionID))
        .run()
        .pipe(Effect.orDie)
      const session = yield* Session.Service
