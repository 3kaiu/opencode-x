    ...
    const ask = Effect.fn("Question.ask")((input: AskInput) =>
      Effect.uninterruptibleMask((restore) =>
        Effect.gen(function* () {
          ...
          return yield* events.publish(Question.Event.Asked, request).pipe(
    ...
    const reply = Effect.fn("Question.reply")((input: ReplyInput) =>
      Effect.uninterruptible(
        Effect.gen(function* () {
          ...
          yield* events.publish(Question.Event.Replied, {
    ...
    const reject = Effect.fn("Question.reject")((requestID: ID) =>
      Effect.uninterruptible(
        Effect.gen(function* () {
          ...
          yield* events.publish(Question.Event.Rejected, {
            sessionID: existing.request.sessionID,
            requestID: existing.request.id,
          })
...
export const node = makeLocationNode({ service: Service, layer, deps: [Event.node] })
