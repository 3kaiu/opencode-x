...
async function attempt<R>(
  ...
  const loadedPlan = candidate.plan
  const filePlugin = pluginSource(loadedPlan.spec) === "file"

  // Deprecated plugin packages are silently ignored because they are now built in.
  if (loadedPlan.deprecated) return { retry: false }

  report?.start?.(candidate, retry)

  const resolved = await resolve(loadedPlan, kind)
  if (!resolved.ok) {
