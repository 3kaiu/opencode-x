...
import { Session as SessionNs } from "@opencode-ai/core/session"
...
import { Workspace as WorkspaceNs } from "@opencode-ai/core/workspace"
...
function workspaceInfo(projectID: Project.ID, type: string, input?: Partial<Workspace.Info>): Workspace.Info {
  return {
    id: input?.id ?? WorkspaceNs.ID.ascending(),
    type,
...
function attachSessionToWorkspace(sessionID: SessionID, workspaceID: WorkspaceNs.ID) {
  return Database.Service.use(({ db }) =>
    db
      .update(SessionTable)
...
describe("workspace schemas and exports", () => {
  ...
  test("validates create input with workspace id, project id, branch, type, and extra", () => {
    const input = {
      id: WorkspaceNs.ID.ascending("wrk_schema_create"),
      type: "worktree",
...
describe("workspace CRUD", () => {
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        ...
        expect(yield* workspace.get(WorkspaceNs.ID.ascending("wrk_missing_get"))).toBeUndefined()
      }),
    { git: true },
  )
    ...
    () =>
      Effect.gen(function* () {
        ...
        const a = workspaceInfo(instance.project.id, "manual", {
          id: WorkspaceNs.ID.ascending("wrk_a_list"),
          branch: "a",
          directory: "/a",
          extra: { a: true },
        })
        const b = workspaceInfo(instance.project.id, "manual", {
          id: WorkspaceNs.ID.ascending("wrk_b_list"),
          branch: "b",
          directory: "/b",
          extra: ["b"],
        })
        const other = workspaceInfo(otherProjectID, "manual", { id: WorkspaceNs.ID.ascending("wrk_c_list") })
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        ...
        const workspaceID = WorkspaceNs.ID.ascending("wrk_create_local")
    ...
    () =>
      Effect.gen(function* () {
        ...
        const existing = workspaceInfo(instance.project.id, type, {
          id: WorkspaceNs.ID.ascending("wrk_list_sync_existing"),
          name: "existing",
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        const workspace = yield* Workspace.Service
        expect(yield* workspace.remove(WorkspaceNs.ID.ascending("wrk_missing_remove"))).toBeUndefined()
      }),
    { git: true },
  )
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        ...
        const info = workspaceInfo(instance.project.id, type, { id: WorkspaceNs.ID.ascending("wrk_remove_throws") })
        registerAdapter(
          instance.project.id,
          type,
          recordedAdapter({
...
describe("workspace waitForSync", () => {
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        const workspace = yield* Workspace.Service
        expect(yield* workspace.waitForSync(WorkspaceNs.ID.ascending("wrk_wait_empty"), {})).toBeUndefined()
      }),
    { git: true },
  )
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        ...
        expect(
          yield* workspace.waitForSync(WorkspaceNs.ID.ascending("wrk_wait_done"), { [sessionID]: 4 }),
        ).toBeUndefined()
        expect(
          yield* workspace.waitForSync(WorkspaceNs.ID.ascending("wrk_wait_done_2"), { [sessionID]: 3 }),
        ).toBeUndefined()
      }),
    { git: true },
  )
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        const workspace = yield* Workspace.Service
        const workspaceID = WorkspaceNs.ID.ascending("wrk_wait_event")
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        const workspace = yield* Workspace.Service
        const workspaceID = WorkspaceNs.ID.ascending("wrk_wait_sync_any")
          ...
          [
            ...
            Effect.gen(function* () {
              ...
              GlobalBus.emit("event", {
                workspace: WorkspaceNs.ID.ascending("wrk_other_workspace"),
                payload: { type: "sync" },
              })
            }),
          ],
      ...
      Effect.gen(function* () {
        ...
        const fiber = yield* Effect.forkChild(
          workspace.waitForSync(
            WorkspaceNs.ID.ascending("wrk_wait_abort"),
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        ...
        expectExitContains(
          yield* Effect.exit(
            workspace.waitForSync(WorkspaceNs.ID.ascending("wrk_wait_timeout"), { [sessionID]: 1 }, undefined, 25),
          ),
