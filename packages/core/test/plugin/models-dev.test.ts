...
describe("ModelsDevPlugin", () => {
  ...
  it.effect("projects declared effort reasoning options as variants on OpenAI-compatible transports", () =>
    Effect.gen(function* () {
      ...
      const model = yield* catalog.model.get(ProviderV2.ID.make("zen"), ModelV2.ID.make("deepseek-v4-flash-free"))
  ...
  it.effect("does not project effort variants for non-OpenAI transports", () =>
    Effect.gen(function* () {
      ...
      const model = yield* catalog.model.get(ProviderV2.ID.make("acme"), ModelV2.ID.make("sonnet-5"))
      expect(model?.variants).toEqual([])
    }),
  )
