...
describe("ModelsDevPlugin", () => {
  ...
  it.effect("projects declared effort reasoning options as variants on OpenAI-compatible transports", () =>
    Effect.gen(function* () {
      const catalog = yield* Catalog.Service
      const models = ModelsDev.Service.of({
        get: () =>
      ...
      const model = yield* catalog.model.get(Provider.ID.make("zen"), Model.ID.make("deepseek-v4-flash-free"))
      expect(model?.variants).toEqual([
        { id: Model.VariantID.make("low"), headers: {}, body: { reasoning_effort: "low" } },
  ...
  it.effect("does not project effort variants for non-OpenAI transports", () =>
    Effect.gen(function* () {
      const catalog = yield* Catalog.Service
      const models = ModelsDev.Service.of({
        get: () =>
      ...
      const model = yield* catalog.model.get(Provider.ID.make("acme"), Model.ID.make("sonnet-5"))
      expect(model?.variants).toEqual([])
    }),
  )
