...
const PluginErrorMessage = Event.define({
  type: "plugin.error",
    ...
    const state = yield* InstanceState.make<State>(
      Effect.fn("Plugin.state")(function* (ctx) {
        ...
        function publishPluginError(message: string) {
          bridge.fork(events.publish(PluginErrorMessage, { message }))
        }
        ...
        const internalInits = yield* Effect.forEach(
        ...
        if (flags.pure && cfg.plugin_origins?.length) {
          yield* Effect.logWarning("plugins ignored in pure mode", { count: cfg.plugin_origins.length })
        }
