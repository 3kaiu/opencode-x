...
function directories(projectID: ProjectNs.ID) {
...
describe("Project directory persistence", () => {
  ...
  it.live("records the active directory under its newly resolved project id", () =>
    Effect.gen(function* () {
      ...
      const remoteID = ProjectNs.ID.make(Hash.fast("git-remote:github.com/project-directory-test/collision"))
  ...
  it.live("clears stale directories when the project id changes", () =>
    Effect.gen(function* () {
      ...
      yield* db
        ...
        .run()
        .pipe(Effect.orDie)
      const remoteID = ProjectNs.ID.make(Hash.fast("git-remote:github.com/project-directory-test/migration"))
      yield* Effect.promise(() =>
