...
function message(providerID: Provider.ID, e: APICallError) {
  return iife(() => {
    const msg = e.message
    if (msg === "") {
...
export function parseAPICallError(input: { providerID: Provider.ID; error: APICallError }): ParsedAPICallError {
