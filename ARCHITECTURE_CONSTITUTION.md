...
| 项 | 基线 |
|---|---|
| core 测试 | 155 测试文件 / 1239 tests：1236 pass / 3 fail（3 项 NpmConfig.registry 为本机 npmmirror 镜像环境类） |
| 包 typecheck | schema/protocol/llm/core/server/opencode/tui/sdk 全绿 |
| 测试运行位置 | 包目录（禁止从仓库根跑） |
| 其他包测试 | schema 32 pass / 0 fail（8 文件）；protocol 2/1；llm 345/32（315 pass + 30 skip）；observability 35/5；server 3/1（httpapi 集成，已接入 CI）；tui 224 pass / 0 fail（含 presentation 卡片投影测试）；web 5 pass / 0 fail（投影契约测试） |
| 批次进度 | A **Completed**；B **Completed**；C **Completed**（core：观测迁包 ✅ → 目录拆分 ✅ → C4 P3.4 ✅ → C7 P3.5 ✅ → C10 P1.4 投影接线 ✅ → C11 P3.1 ✅ → C12 goal 节点 ✅ → C13 三件套 ✅ → C15 retrospect 命令 ✅ → 全模块观测 ✅；跨批次依赖：C12 drift 消费=协议/命令面，C15 复盘命令为 CLI）；D **Completed**（plugin v1 退役评估 ✅；server 端点唯一性 ✅；请求 span ✅ http.request metrics；cursor 收敛评估 ✅）；E **Completed**（v1 运行栈删除 ✅ → 组合根组装 + 观测装配 ✅；删除后清理 ✅：孤儿测试 7 处/死 patch 模块/TODO 5 处/app-node-builder 命名/调试写文件 2 处；server httpapi 测试修复 + CI 接入；plugin v1 四出口 deprecated 标记完成（ADR-015）；opencode 1527 pass，15 失败为环境类，其中权限/attach/SSE 三项已随批次 H 修复）；F **Completed**（单存储收敛 + v2-bridge 删除 ✅；视图拆分 ✅；verify 视图 ✅ / plan·cost 轻量形态（验收口径）；i18n zh/en ✅；渲染观测 ✅；测试全绿）；G **Completed**（server.ts 合并 ✅；v2 面再生成 diff 干净 ✅；typecheck 绿）；H **Completed**（跨链组装 + DeepSeek Harness 架构吸纳：Batch 1 Tool Guard 单调权限 ✅（ADR-016）；Batch 2 ignorable 事件标记 + 压缩收缩不变式 ✅（ADR-017）；Batch 3 呈现契约化 + web 呈现包 ✅（ADR-018：schema ToolPresentation ✅ → core present 接线 + builtins ✅ → SDK 再生成 ✅ → tui 投影/卡片 ✅ → packages/web ✅ → serve 静态托管 ✅ 已随二进制内嵌 + ServerAuth 鉴权 E2E 验证）；Batch 3 其余存档候选——溢出恢复前进校验/计数、usage-anchored 压力计量、压缩事务锁、先剪枝后摘要；Batch H 收尾 ✅：checkpoint 回归修复——core Project Service tag 冲突（ProjectV2 与 Project 双 tag）导致 instance initGit 500，已恢复 core `@opencode/ProjectV2` tag 与 server 装配；MCP SDK SSE error-reconnect patch（#39697）在 #39373 重写时丢失已恢复；批次 H 内部 3 处回归已修复——readAggregate 分页按过滤后事件计数（33fa5312d3 引入，gap 事件错位/重复）、Permission pending 全局化后测试固定 ID 泄漏改为唯一 ID 并清理（3210f2599e 引入）、turn 结算事件与投影解耦（c4843a9110/a44e477755 引入：空流回合照发 unknown Step.Ended 保持 CLI 事件契约，projector 在 Step.Ended 投影时剪掉 finish=unknown 且无 content 的空 assistant 消息，durable history 不再出现幽灵消息——session-runner 94 pass 与 run-process 13 pass 双契约同时成立）；close-out 验证修正：httpapi-listen 5s 默认超时过紧导致负载 flaky → 实例启动类测试显式 15s；该文件 default in-process 测试原命中 UI 兜底路由在 dev 树（无内嵌 UI bundle）代理到 app.opencode.ai 造成网络依赖挂起 → 改用纯本地 /doc 路由；实测基线——core 1236 pass / 3 fail（155 文件 / 1239 tests，NpmConfig.registry npmmirror 环境类），opencode 1540 pass / 14 skip / 3 fail（1557 tests / 149 文件，installation registry 3 项 npmmirror 环境类）） |
