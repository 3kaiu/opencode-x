...
describe("WebSearchTool registration", () => {
  it.effect("registers websearch, asserts query permission, and calls Exa", () =>
    Effect.gen(function* () {
      ...
      expect((yield* toolDefinitions(registry)).map((tool) => tool.name)).toEqual(["websearch", "get_tool_schema"])
      expect(
  ...
  it.effect("rejects oversized MCP response bodies", () =>
    Effect.gen(function* () {
      ...
      ).toEqual({ type: "error", value: "Unable to search the web for too much [aci: Unknown, retry: retry]" })
