...
describe("plugin.auth-override", () => {
  it.instance(
    "user plugin overrides built-in xai auth",
    () =>
      Effect.gen(function* () {
        ...
        yield* fs.writeWithDirs(
          path.join(pluginDir, "custom-xai-auth.ts"),
          [
            "export default {",
            '  id: "demo.custom-xai-auth",',
            "  server: async () => ({",
            "    auth: {",
            '      provider: "xai",',
            "      methods: [",
        ...
        const plugin = pathToFileURL(path.join(pluginDir, "custom-xai-auth.ts")).href
