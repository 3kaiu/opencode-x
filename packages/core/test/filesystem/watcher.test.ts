...
const it = testEffect(AppNodeBuilder.build(LayerNode.group([FSUtil.node, Event.node])))
...
describeWatcher("Watcher", () => {
  ...
  it.live("cleanup stops publishing events", () =>
    ...
    }).pipe(Effect.provide(AppNodeBuilder.build(LayerNode.group([FSUtil.node, Event.node])))),
  )
